...
namespace Hast.Samples.Posit
{
    ...
    public class Posit32_0_Calculator
    {
        ...
        public virtual void CalculatePowerOfReal(SimpleMemory memory)
        {
            ...
            var a = new Posit32_0(positToMultiply, true);
            var b = a;
        ...
        public virtual void AddPositsInArray(SimpleMemory memory)
        {
            ...
            var result = new Posit32_0((uint)memory.ReadUInt32(AddPositsInArray_InputPosit32sStartIndex), true);

            for (int i = 1; i < numberCount; i++)
            {
                result += new Posit32_0((uint)memory.ReadUInt32(AddPositsInArray_InputPosit32sStartIndex + i), true);
            }
    ...
    public static class Posit32_0_CalculatorCalculatorExtensions
    {
            ...
            IHardwareGenerationConfiguration configuration = null)
        {
            if (numbers.Length !=  Posit32_0_Calculator.MaxDegreeOfParallelism)
            {
                throw new ArgumentException(
                    "Provide as many numbers as the degree of parallelism of  Posit32_0_Calculator is (" +
            ...
            {
                memory.WriteInt32( Posit32_0_Calculator.ParallelizedCalculateLargeIntegerSum_Int32NumbersStartIndex + i, numbers[i]);
            }

            positCalculator.ParallelizedCalculateIntegerSumUpToNumbers(memory);

            var results = new int[ Posit32_0_Calculator.MaxDegreeOfParallelism];

            for (int i = 0; i < numbers.Length; i++)
            {
                results[i] = memory.ReadInt32( Posit32_0_Calculator.ParallelizedCalculateLargeIntegerSum_OutputInt32sStartIndex + i);
            }
            ...
            IHardwareGenerationConfiguration configuration = null)
        {
            ...
            {
                memory.WriteUInt32( Posit32_0_Calculator.AddPositsInArray_InputPosit32sStartIndex + i, positArray[i]);
            }

            positCalculator.AddPositsInArray(memory);

            return (float)new Posit32(memory.ReadUInt32( Posit32_0_Calculator.AddPositsInArray_OutputPosit32Index), true);
        }
    }
}
