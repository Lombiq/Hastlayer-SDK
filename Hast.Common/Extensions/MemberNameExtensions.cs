...
namespace Hast.Common.Extensions
{
    ...
    public static class MemberNameExtensions
    {
        ...
        public static IEnumerable<string> GetMemberNameAlternates(this string memberFullName)
        {
            ...
            {
                // 1. alternate:
                sides[0] + "::" + sides[1][sides[1].IndexOf(methodName + "(", StringComparison.InvariantCulture)..],
                // 2. alternate:
                returnType + " " + sides[1].Replace("." + methodName + "(", "::" + methodName + "(", StringComparison.InvariantCulture),
            };
        }
    }
}
