...
public interface IHastlayer : IDisposable
{
    ...
    /// Configuration for how the proxy generation should happen. If null, it will be substituted with <see
    /// cref="ProxyGenerationConfiguration.Default"/>.
    /// </param>
    ...
    /// container that maintains an independent dependency injection scope as long as the host <see cref="IHastlayer"/>
    /// object isn't disposed.
    /// </summary>
    ...
    /// <summary>
    /// Constructs a new <see cref="SimpleMemory"/> object that represents a simplified memory model available on the
    /// FPGA for transformed algorithms.
    /// </summary>
    ...
    /// allocated memory space is calculated from the cell count and the cell size indicated in <see
    /// cref="SimpleMemory.MemoryCellSizeBytes"/>.
    /// </param>
    ...
    /// <summary>
    /// Constructs a new <see cref="SimpleMemory"/> object that represents a simplified memory model available on the
    /// FPGA for transformed algorithms. Use this method only if you have no control over the source of the input data
    /// or if the generation process can't be done through Memory&lt;byte&gt; access alone. (E.g. if using an API that
    /// only supports classic array manipulation) Otherwise prefer to use the other overload and produce the content
    /// using Memory&lt;byte&gt; operations. Use <see cref="SimpleMemoryAccessor"/> to gain access to the internal
    /// Memory&lt;byte&gt;.
    /// </summary>
    /// <param name="configuration">The configuration with the device information.</param>
    /// <param name="data">
    /// The input data which is referenced by or copied into the <see cref="SimpleMemory"/> depending on the selected
    /// device's characteristics, particularly its <c>MemoryConfiguration.MinimumPrefix</c>.
    /// </param>
    /// <param name="withPrefixCells">The amount of empty header space reserved in the <paramref name="data"/>.</param>
    /// <returns>A new instance of <see cref="SimpleMemory"/>.</returns>
    // Here we use Memory&lt;byte&gt; because xmldoc doesn't support generics with a definite type in <see> and in the
    // generated documentation they get replaced with <T>. See https://stackoverflow.com/a/41208166.
