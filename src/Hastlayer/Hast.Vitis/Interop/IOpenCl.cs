...
/// <see href="https://www.khronos.org/registry/OpenCL/sdk/1.0/docs/man/xhtml/"/>.
/// </summary>
...
public interface IOpenCl : IDisposable
{
    ...
    /// The number of cl_platform_id entries that can be added to platforms. If platforms is not NULL, the num_entries
    ...
    /// An enumeration constant that identifies the platform information being queried. It can be one of the following
    ...
    /// Returns the actual size in bytes of data being queried by param_value. If param_value_size_ret is NULL, it is
    /// ignored.
    /// </param>
    ...
    /// devices or all OpenCL devices available. The valid values for device_type are specified in the following table.
    /// </param>
    /// <param name="numberOfEntries">
    /// The number of cl_device entries that can be added to devices. If devices is not NULL, the num_entries must be
    /// greater than zero.
    /// </param>
    /// <param name="devices">
    /// A list of OpenCL devices found. The cl_device_id values returned in devices can be used to identify a specific
    /// OpenCL device. If devices argument is NULL, this argument is ignored. The number of OpenCL devices returned is
    /// the minimum of the value specified by num_entries or the number of OpenCL devices whose type matches
    /// device_type.
    /// </param>
    ...
    /// followed by the corresponding desired value. The list is terminated with 0. properties can be NULL in which case
    /// the platform that is selected is implementation-defined. The list of supported properties is described in the
    /// table below.
    /// </param>
    /// <param name="numberOfDevices">The number of devices specified in the devices argument.</param>
    /// <param name="devices">A pointer to a list of unique devices returned by clGetDeviceIDs for a platform.</param>
    ...
    /// implementation to report information on errors that occur in this context. This callback function may be called
    ...
    /// </param>
    /// <param name="userData">Passed as the user_data argument when pfn_notify is called. user_data can be NULL.</param>
    ...
    /// specified in the table below can be set in properties; otherwise the value specified in properties is considered
    /// to be not valid.
    /// </param>
    /// <param name="errorCode">
    /// Returns an appropriate error code. If errcode_ret is NULL, no error code is returned.
    ...
    /// The number of devices listed in device_list. The devices associated with the program object will be the list of
    /// devices specified by device_list. The list of devices specified by device_list must be devices associated with
    /// context.
    /// </param>
    /// <param name="lengths">
    ...
    /// length of this corresponding binary is given by lengths[i]. lengths[i] cannot be zero and binaries[i] cannot be
    /// a NULL pointer. The program binaries specified by binaries contain the bits that describe the program executable
    /// that will be run on the device(s) associated with context. The program binary can consist of either or both of
    /// device-specific executable(s), and/or implementation-specific intermediate representation (IR) which will be
    /// converted to the device-specific executable.
    /// </param>
    /// <param name="binaryStatus">
    /// Returns whether the program binary for each device specified in device_list was loaded successfully or not. It
    /// is an array of num_devices entries and returns CL_SUCCESS in binary_status[i] if binary was successfully loaded
    /// for device specified by device_list[i]; otherwise returns CL_INVALID_VALUE if lengths[i] is zero or if
    /// binaries[i] is a NULL value or CL_INVALID_BINARY in binary_status[i] if program binary is not a valid binary for
    /// the specified device. If binary_status is NULL, it is ignored.
    /// </param>
    /// <param name="errorCode">
    /// Returns an appropriate error code. If errcode_ret is NULL, no error code is returned.
    /// </param>
    ...
    /// built for all devices associated with program for which a source or binary has been loaded. If device_list is a
    /// non-NULL value, the program executable is built for devices specified in this list for which a source or binary
    /// has been loaded.
    /// </param>
    /// <param name="deviceList">The number of devices listed in device_list.</param>
    /// <param name="options">
    /// A pointer to a string that describes the build options to be used for building the program executable. The list
    /// of supported options is described in "Build Options" below.
    /// </param>
    /// <param name="notify">
    /// A function pointer to a notification routine. The notification routine is a callback function that an
    /// application can register and which will be called when the program executable has been built (successfully or
    /// unsuccessfully). If pfn_notify is not NULL, clBuildProgram does not need to wait for the build to complete and
    /// can return immediately. If pfn_notify is NULL, clBuildProgram does not return until the build has completed.
    /// This callback function may be called asynchronously by the OpenCL implementation. It is the application's
    /// responsibility to ensure that the callback function is thread-safe.
    /// </param>
    /// <param name="userData">Passed as an argument when pfn_notify is called. user_data can be NULL.</param>
    ...
    /// n - 1, where n is the total number of arguments declared by a kernel.
    /// </param>
    /// <param name="argumentSize">
    /// Specifies the size of the argument value. If the argument is a memory object, the size is the size of the buffer
    /// or image object type. For arguments declared with the __local qualifier, the size specified will be the size in
    /// bytes of the buffer that must be allocated for the __local argument. If the argument is of type sampler_t, the
    /// arg_size value must be equal to sizeof(cl_sampler). For all other arguments, the size will be the size of
    /// argument type.
    /// </param>
    /// <param name="argumentValue">
    /// A pointer to data that should be used as the argument value for argument specified by arg_index. The argument
    /// data pointed to by arg_value is copied and the arg_value pointer can therefore be reused by the application
    /// after clSetKernelArg returns. The argument value specified is the value used by all API calls that enqueue
    /// kernel (clEnqueueNDRangeKernel and clEnqueueTask) until the argument value is changed by a call to
    ...
    /// A valid command-queue. The specified set of memory objects in mem_objects will be migrated to the OpenCL device
    ...
    /// <param name="memoryMigrationFlags">A bit-field that is used to specify migration options.</param>
    /// <param name="numberOfEventsInWaitList">The length of <paramref name="eventWaitList"/>.</param>
    /// <param name="eventWaitList">
    /// Specify events that need to complete before this particular command can be executed. If event_wait_list is NULL,
    /// then this particular command does not wait on any event to complete. If event_wait_list is NULL,
    ...
    /// this particular command to complete. event can be NULL in which case it will not be possible for the application
    /// to query the status of this command or queue a wait for this command to complete. If the event_wait_list and the
    /// event arguments are not NULL, the event argument should not refer to an element of the event_wait_list array.
    /// </param>
    ...
    /// </param>
    /// <param name="waitEvent">
    /// Returns an event object that identifies this particular kernel execution instance. Event objects are unique and
    /// can be used to identify a particular kernel execution instance later on. If event is NULL, no event will be
    /// created for this kernel execution instance and therefore it will not be possible for the application to query or
    ...
    /// <summary>
    /// Blocks until all previously queued OpenCL commands in a command-queue are issued to the associated device and
    /// have completed.
    /// </summary>
