...
/// <summary>
/// Showcasing some simple recursive algorithms, since recursive method calls are also supported by Hastlayer. Do note
/// however that recursive algorithms is not the best use-case of Hastlayer and should generally be avoided.
///
...
/// were invoked. This is only interesting when debugging the hardware side: writing debug data to the memory like this
/// can give you insights into how your code works on the hardware (this is even more useful if you also debug with the
/// FPGA SDK and check the memory of the FPGA there). Note that naturally adding such memory operations makes the
/// algorithm much slower.
/// </summary>
/// <remarks>
...
public class RecursiveAlgorithms
{
    ...
    // The return value should be a type with a bigger range than the input. Although we can use 64b numbers internally
    // we can't write the to memory yet so the input needs to be a short.
