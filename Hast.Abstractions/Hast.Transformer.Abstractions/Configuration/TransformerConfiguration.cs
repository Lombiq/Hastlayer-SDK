...
#pragma warning restore S2365 // Properties should not make collection or array copies
    ...
    /// space to a byte array. Defaults to <see langword="true"/>.
    /// </summary>
    public bool UseSimpleMemory { get; set; } = true;

    /// <summary>
    /// Gets or sets a value indicating whether inlining methods on the hardware are enabled. Defaults to <see
    /// langword="true"/>. Inlining methods eliminates the performance impact of method calls (and is thus advised for
    /// small, frequently invoked methods), but causes the hardware design to be larger (and hardware generation to be
    /// slower). Be aware that inlining, even if enabled, doesn't happen automatically, check the documentation.
    /// </summary>
    public bool EnableMethodInlining { get; set; } = true;

    /// <summary>
    /// Gets the list of methods that should be inlined in addition to methods already marked with a suitable
    ...
    /// Hastlayer can figure out what the compile-time size of an array is most of the time. Should this fail you can
    /// use this to specify array lengths.
    ///
    /// Key should be the full name of the array ( <see
    /// cref="IHardwareGenerationConfiguration.HardwareEntryPointMemberFullNames"/>) and value should be the length. If
    /// you get exceptions due to arrays missing their sizes the exception will indicate the full array name too.
    /// </summary>
    public IDictionary<string, int> ArrayLengths { get; } = new Dictionary<string, int>();

    /// <summary>
    /// Gets or sets a value indicating whether interfaces that are implemented by transformed types are processed.
    /// Currently such interfaces don't affect the resulting hardware implementation, but the assemblies of all
    /// referenced interfaces need to be loaded. If set to <see langword="false"/> such loading is not necessary.
    /// Defaults to <see langword="false"/>.
    /// </summary>
    public bool ProcessImplementedInterfaces { get; set; }

    /// <summary>
    /// Gets or sets a value indicating whether constant values are propagated through the processed code and variables
    /// that actually hold only these values are substituted with the values themselves (also called constant folding or
    /// constant propagation). This can significantly cut down on the code complexity and improve performance, but in
