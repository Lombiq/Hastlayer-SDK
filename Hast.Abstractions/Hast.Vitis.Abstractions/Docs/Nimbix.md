...
1. Run the host executable configured for the selected Vitis device to generate the RTL source as VHDL. It will terminate with an exception indicating that the Vitis environment is missing or "The OpenCL binary (xclbin) is required to start the kernel. The host can't launch without it." This is normal since you'll run the app on the Nimbix server. (You can also do this already on the Nimbix machine but it's simpler locally).
