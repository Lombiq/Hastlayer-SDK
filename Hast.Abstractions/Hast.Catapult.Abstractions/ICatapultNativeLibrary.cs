...
/// </summary>
/// <param name="logflag">
/// The value of a <see cref="Constants.Log"/> flag enum converted into uint. This can be used to filter out specific
/// types of messages, eg <see cref="Constants.Log.Verbose"/>.
/// </param>
...
public interface ICatapultNativeLibrary : IDisposable
{
    ...
    /// image. Writing to the golden image requires explicitly passing in <see cref="Constants.HandleFlag.WriteGolden"/>
    ...
    /// an ANSI string, which contains a single log entry. The <see cref="Constants.Log"/> enumeration is derived from
    /// the FPGA_LOG_* flags in the original library define all the possible (and non-mutually-exclusive) severity
    /// levels.
    /// </param>
    /// <returns>
    /// <see cref="Constants.Status.Success"/> if the device is present or <see
    /// cref="Constants.Status.HardwareNotPresent"/> if the device is not present.
    /// </returns>
